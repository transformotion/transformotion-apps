...
import type { EtfMarket, EtfPick, EtfsData } from '../../types/stock';
...
export function EtfsPage() {
  const { market, data, cachedAt } = useTabStore(s => s.etfs);
  const setEtfs = useTabStore(s => s.setEtfs);
  ...
  useEffect(() => {
    if (data) return;
    const newKey = CK.etfs(market);
    const oldKey = `ETFS_${market}`;
    void (async () => {
      const cached = await getCache<EtfsData>(newKey) ?? await getCache<EtfsData>(oldKey);
      if (cached) setEtfs({ data: cached.data, cachedAt: cached.cachedAt });
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const fetchEtfs = useCallback(async (selectedMarket: EtfMarket, forceRefresh = false) => {
    const cacheKey = CK.etfs(selectedMarket);

    if (!forceRefresh) {
      const cached = await getCache<EtfsData>(cacheKey);
      if (cached && isCacheFresh(cached.cachedAt, 'etfs')) {
        setEtfs({ data: cached.data, cachedAt: cached.cachedAt });
        return;
      }
    }
...
Include a range covering at least one property/REIT ETF and one bond/defensive ETF alongside growth options.
Return ONLY valid JSON. No markdown, no XML tags, no citation tags in any field value:
{"market":"${selectedMarket}","asOf":"Month YYYY","stocks":[{"ticker":"","companyName":"Full ETF name","sector":"e.g. Broad Index / Property & REITs / Bonds","verdict":"BUY or HOLD","currentPrice":"","priceChange":"e.g. +1.2%","reason":"2 sentences — why this ETF stands out now, include expense ratio."}]}`;

    const result = await callClaude<EtfsData>({ prompt, webSearch: true });
    const now = new Date().toISOString();
    setEtfs({ data: result, cachedAt: now });
    await putCache(cacheKey, result, isLive ? 'live' : 'fast', 'etfs');
  }, [callClaude, getCache, putCache, isLive, setEtfs]);

  function handleMarketChange(m: EtfMarket) {
    setEtfs({ market: m, data: null, cachedAt: null });
  }
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
        ...
        {cachedAt && !loading && <CacheStatusBadge cachedAt={cachedAt} type="etfs" />}
      </div>

      {error && <ErrorBox message={error} />}
      {loading && <LoadingSpinner message="Finding top ETF picks…" subMessage={statusMessage} />}
