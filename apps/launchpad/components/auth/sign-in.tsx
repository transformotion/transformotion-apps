  ...
  return (
    <>
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md p-6 bg-card border border-border rounded-2xl shadow-2xl z-50">
        <h3 className="text-lg font-semibold text-foreground mb-2">Forgot how you signed in?</h3>
        {submitted ? (
          <div className="py-4">
            <p className="text-sm text-muted-foreground">
              We&apos;ve sent a hint to{' '}
              <span className="text-foreground font-medium">{email}</span> about which sign-in
              method you used.
            </p>
            <button
              onClick={() => {
          ...
          <form onSubmit={handleSubmit}>
            <p className="text-sm text-muted-foreground mb-4">
              Enter your email and we&apos;ll send you a hint about which provider you used to sign
              in.
            </p>
            <input
              type="email"
...
export function SignIn({ onSignIn }: { onSignIn?: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <ForgotProviderModal isOpen={forgotProviderOpen} onClose={() => setForgotProviderOpen(false)} />
    </div>
  )
}
