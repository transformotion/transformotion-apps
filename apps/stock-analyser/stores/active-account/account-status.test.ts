...
describe('resolveAccountState — error vs no-access must be distinct (#415 addition 2)', () => {
  it('control-plane FAILURE → error (retry), NEVER no-access', () => {
    // Even with no accounts, a failed fetch is `error`, not `no-access`:
    // an outage must not look like revocation.
  ...
  it('selection present for stock-analyser → ready with that account', () => {
    const out = resolveAccountState(accounts('acc-1', 'acc-2'), {
      ok: true,
      selections: [
        { appSlug: 'budget-tracker', accountId: 'bt-1' },
        { appSlug: 'stock-analyser', accountId: 'acc-2' },
      ],
    })
  ...
  it('member but no stored selection → ready with the first account', () => {
    const out = resolveAccountState(accounts('acc-1', 'acc-2'), {
      ok: true,
      selections: [{ appSlug: 'budget-tracker', accountId: 'bt-1' }],
    })
    expect(out.status).toBe('ready')
    expect(out.activeAccountId).toBe('acc-1')
  })

  it('ignores a selection for another app', () => {
