...
1. Open the Stock Analyser-owned WebSocket (`NEXT_PUBLIC_SA_WSS_URL`) with Cognito ID token
...
NEXT_PUBLIC_SA_WSS_URL=wss://...               # Stock Analyser WebSocket URL for job notifications
```
...
- Opens Stock Analyser WebSocket (`NEXT_PUBLIC_SA_WSS_URL`) with Cognito ID token
