...
const APP_NAME     = process.env.APP_NAME ?? 'budget-tracker';
...
export const handler = async (event: WsAuthorizerEvent): Promise<AuthorizerResult> => {
  ...
  if (!token) {
    console.log('[bt-ws-authorizer] rejected: no token in query string');
    return makePolicy('Deny', event.methodArn, 'anonymous');
  }

  if (requestedApp !== APP_NAME) {
    console.log(`[bt-ws-authorizer] rejected: app '${requestedApp}' is not '${APP_NAME}'`);
    return makePolicy('Deny', event.methodArn, 'anonymous');
  }
  ...
  try {
    ...
    if (accountId && !appAccountIds.includes(accountId)) {
      console.log(`[bt-ws-authorizer] rejected: accountId ${accountId} not in user's ${APP_NAME} accounts`);
      return makePolicy('Deny', event.methodArn, userId);
    }

    console.log(`[bt-ws-authorizer] allowed: userId=${userId} accountId=${accountId ?? 'none'}`);
    ...
    console.log('[bt-ws-authorizer] rejected: token verification failed:', (err as Error).message);
    return makePolicy('Deny', event.methodArn, 'anonymous');
  }
};
