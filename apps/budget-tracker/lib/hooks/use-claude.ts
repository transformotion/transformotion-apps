...
export function useClaude<T = unknown>(options: UseClaudeOptions = {}): UseClaudeReturn<T> {
  ...
  const callClaude = useCallback(async (request: ClaudeRequest): Promise<T> => {
    ...
    try {
      ...
      // Real implementation: POST to Claude API proxy
      ...
      // Poll for result
      const result = await pollForResult<T>(
        newJobId,
        config.claude.cacheUrl,
        pollInterval,
        maxPollTime,
      ...
      if (err instanceof Error && (err.name === 'AbortError' || err.message === 'Request aborted')) {
        // Silently ignore abort errors - a new request is likely in progress
...
/**
 * Poll the analysis-cache endpoint until job completes
 */
async function pollForResult<T>(
  ...
  while (Date.now() - startTime < maxPollTime) {
    ...
    const response = await fetch(`${cacheUrl}/job-${jobId}`, { signal })
    
    if (response.ok) {
      const status: ClaudeJobStatus<T> = await response.json()
      
      if (status.status === 'complete' && status.content !== undefined) {
