/**
 * Pure status-decision logic for the Budget Tracker active-account store.
 * Framework/auth-free so it can be unit-tested without the Amplify chain.
 * (Mirror of the Stock Analyser equivalent, app-slug aside.)
 */

export const APP_SLUG = 'budget-tracker'
...
/**
 * Decide the access state (error must stay DISTINCT from no-access):
 *  - fetch FAILED            → `error` (retry)
 *  - has a selection for BT  → `ready`
