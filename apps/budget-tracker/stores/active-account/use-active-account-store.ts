...
export const useActiveAccountStore = create<ActiveAccountState>((set, get) => ({
  ...
  load: async () => {
    ...
    // Full account list (for the selector) from the token claim — no network.
    ...
    try {
      ...
      // Control-plane FAILURE → error/retry, never no-access.
      console.warn('[budget-tracker] active-account load failed:', err)
      result = { ok: false }
    ...
    // Best-effort: enrich labels with real account names (cosmetic; never affects access).
  ...
  switchTo: async (accountId: string) => {
    ...
    if (!cp) return
    try {
      await cp.setActiveAccount(APP_SLUG, accountId) // server fails closed on non-membership
    } catch (err) {
      console.warn('[budget-tracker] active-account switch failed:', err)
