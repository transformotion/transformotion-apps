...
App stacks (`deploy-stock-analyser.yml`, `deploy-budget-tracker.yml`, `deploy-migration-utilities.yml`) consume the CF exports produced in Step 3/4 above where they still have transitional dependencies. Budget Tracker owns `Transformotion{Stage}-BudgetTrackerWs`, `Transformotion{Stage}-BudgetTrackerApi`, and `budget-tracker-ai-proxy-{stage}` after #367 and no longer uses platform REST/WSS/Claude runtime for live Budget Tracker flow. Stock Analyser owns `Transformotion{Stage}-StockAnalyserWs`, `Transformotion{Stage}-StockAnalyserApi`, and `stock-analyser-ai-proxy-{stage}` after #366, and no longer uses platform REST/WSS/Claude runtime for live AI flow. During #386 auth cutover, app deploy workflows keep `LAUNCHPAD_AUTH_CUTOVER_ENABLED=false` until the explicit cutover deploy; setting it to `true` makes SA/BT API and WSS authorizers import `LaunchpadAuth` instead of `AuthStack`. Platform deploys do not cascade into app workflows; each app/utility workflow is independently triggered and independently deploys only its own stacks.
...
Control-plane and platform Lambda environment variables:
