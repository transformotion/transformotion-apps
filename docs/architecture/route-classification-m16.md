...
| `POST /accounts` | accounts | ~~withAuth~~ → `withAuthOnly` | `auth-only` + **requires `{appSlug}-app-access` group** (M11 A4) | **PR-6A: withAuthOnly (create has no account context). M11 A4: request `{ name, appSlug }` (m16.6.0 — appSlug in body, not `aud`, so the launchpad token works); gate on `{appSlug}-app-access` (body says which app, group authorizes), creator→owner, ensure access group** | Resolved | PR-6A / **M11 A4 / m16.6.0** |
| `POST /api/invitations/bundles` | invitation-bundles | (was Phase 8) | `withAuthOnly` + **canCreateInvitationGrant** per grant (site-admin / `{app}-app-admin`); same-app conflict; partial acceptance | **M11 Chunk 3 (Option A): built — the composer's real backend; two kinds (m16.5.0); sender-authorized per grant; grants conferred at redemption** | **Implemented (M11 Chunk 3)** | **M11 Chunk 3** |
| `POST /api/invitations/bundles/{bundleId}/redeem` | invitation-redemption | (unbuilt) | `withAuthOnly` + **invitee-only**; per-grant `account-invite` / `app-grant` redemption | **M11 A5: built — membership+access (account-invite) / access-only (app-grant); idempotent; SES delivery is a separate seam** | **Implemented (M11 A5)** | **M11 A5** |
| `POST /api/invitations/bundles/{bundleId}/redeem-as` | invitation-redemption | (new) | DEV-ONLY demo bypass (impersonation); **server-side STAGE guard** (refused in prod) + site-admin caller | **M11 Chunk 3: built — drives the real A5 path in dev before SES; the prod boundary is the deployed env, not the client `NEXT_PUBLIC_DEV_TOOLS` flag** | **Implemented (M11 Chunk 3, dev-only)** | **M11 Chunk 3** |
...
| `PUT/DELETE …/ai-runtime-config/apps/{appSlug}/override` | ai-runtime-config | `requireAppAdminForApp` (token `{app}-app-admin` group) | `operational-config` → **app-admin override** (D9) | per-app override; site-admin override-write removed (D9 correction) | **Implemented (M11 A3)** | M11 A2/A3 |
