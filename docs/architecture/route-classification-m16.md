...
| `POST /accounts` | accounts | ~~withAuth~~ → `withAuthOnly` | `auth-only` + **requires `{app}-app-access` group** (M11 A4) | **PR-6A: withAuthOnly (create has no account context). M11 A4: gate on the app-access group (groups-authoritative), appSlug from `aud`, creator→owner, ensure access group** | Resolved | PR-6A / **M11 A4** |
