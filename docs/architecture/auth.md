...
| `apps/stock-analyser/functions/notification-history` | `withAuthOnly` | **Server-side projection IS the authorization** (#573): every caller reads, the WIRE PAYLOAD is scoped per viewer via `projectRunHistoryForViewer` over LIVE membership — admin→summaries, owner/manager→own-account detail, member/viewer→nothing; **fail-closed** on membership-lookup error (no detail). A baseline `require*` does not fit (a site-admin who is not an SA member must still see all runs). Exempt from the handler-authz-pattern check; compensating control is the payload-scoping test suite. | `stock-analyser.notification-send-log` R + `launchpad-account-members` `userId-index` Query |
