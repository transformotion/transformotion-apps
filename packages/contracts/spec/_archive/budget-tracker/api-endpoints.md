...
```typescript
{
  categoryId?: string;            // UUID — must reference a valid category for this account
  subcategoryId?: string;         // UUID — must belong to categoryId
...
Get the full budget domain for the account: categories tree, budget amounts, budget frequencies.
...
### `PATCH /api/budget/budget-data`
Partial update — send any subset of BudgetData fields.

**Request:** any subset of `BudgetData` fields.
```typescript
{
  categories?: Category[];
  budgetAmounts?: { [subcategoryId: string]: number };
  budgetFrequencies?: { [subcategoryId: string]: Frequency };
}
```

**Validation:**
- All `subcategoryId` keys in `budgetAmounts`/`budgetFrequencies` must reference subcategories in the account's category tree.
- Category writes: each category must have at least one non-deleted subcategory.
- Subcategory deletion: only soft-delete (`deleted: true`). Rejecting hard-deletes.
...
(Formerly "Custom Rules".)

### `GET /api/budget/rules`
List matching rules for the account.
...
Get settings for the account (CSV format mappings only).
...
**Request:** any subset of `BudgetSettings` fields (currently only `csvFormatMappings`).
