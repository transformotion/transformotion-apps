...
| 0.4 | Bootstrap AWS CDK project in `infra/` with dev and prod stacks | CDK app compiles |
...
| 1.3 | Auth flows: Sign Up, Confirm Email, Sign In, Forgot Password, Sign Out — custom styled components matching dark theme | All auth screens |
...
| 1.7 | Deploy frontend to S3 + CloudFront via CDK; ACM cert; custom domain; update DNS CNAME | HTTPS live on `apps.transformotion.com.au` |
...
| S1.7 | CloudFront deploy | S3 + CloudFront; ACM cert; custom domain; DNS CNAME pointed to distribution |
...
|---------|---------|--------|
| Frontend | Single HTML file | React + Vite + TypeScript |
| Hosting | `localhost:3000` | S3 + CloudFront |
| Auth | None | Cognito (Groups + custom attributes) |
| Account model | None | UUID-based accounts with members and roles |
| API key | localStorage | AWS Secrets Manager (`/prod/anthropic/api-key`) |
| Claude API calls | Direct from browser | Server-side via Lambda proxy (from Phase 2) |
| Portfolio storage | localStorage + portfolio-cache.json | DynamoDB `stock-analyser.portfolio` (account-scoped) |
| Watchlist storage | localStorage | DynamoDB `stock-analyser.watchlist` (account-scoped) |
| Analysis cache | localStorage | DynamoDB `stock-analyser.cache` with TTL (account-scoped) |
| Budget Tracker data | localStorage | DynamoDB `budget-tracker.*` with one-click migration |
| Email alerts | Local node-cron + Nodemailer | EventBridge + Lambda + SES |
| DNS | Existing registrar (to be confirmed) | Same registrar; CNAME added for `apps` subdomain |
| CI/CD | None (manual git push) | GitHub Actions (OIDC, no long-lived keys) |
| Observability | None | CloudWatch |
